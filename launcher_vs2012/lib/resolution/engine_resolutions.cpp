...
struct screen find_resolution(int res_id)
{
    ...
    for (i = 1; EnumDisplaySettings(NULL, i, &resolution_struct) != 0; ++i)
    {
		...
		if((width < RESOLUTION_MIN_X || height < RESOLUTION_MIN_Y) || ((width > RESOLUTION_MAX_X || height > RESOLUTION_MAX_Y) && get_ini(ini_fName).resolution_lock == true))
		{
			continue;
		}
    ...
    // write the whole thing into acklog
    //DEBUGLOG("Primary monitor resolutions: " << "\n");
    //for (i = 0; i < total_resolutions; i++)
    //{
        //DEBUGLOG(i+1 << "/" << total_resolutions << " " <<resolution_available_list[RESOLUTION_WIDTH][i] << "x" << resolution_available_list[RESOLUTION_HEIGHT][i] << "\n");
    //}
	resolution.total_screen = total_resolutions;
	resolution.x = resolution_available_list[RESOLUTION_WIDTH][res_id];
	resolution.y = resolution_available_list[RESOLUTION_HEIGHT][res_id];
	return resolution;
}